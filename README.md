...
| M09 / M12 franchise compilers               | Same approach as M08 with per-game base year + own templates              | ✅ Built end-to-end for 2018: M09 SEYR=10 (base 2008), M12 SEYR=7 (base 2011), both at $177.2M cap. M09 PCSX2-verified after roster-compiler upgrades.                                                                                       |
| Roster compiler upgrades (Tier 16)          | New PGIDs + appearance + DCHT rebuild + stale-PGID safety sweep           | ✅ Each canonical player gets a fresh PGID (16384-32767) to avoid Madden's baked photo library. Hash-derived appearance fields. DCHT rebuilt with L/R distribution. 50+ PGID-keyed tables cleared to prevent orphan-pointer crashes.         |
| Data-integrity audit (Tier 17)              | Fixed 5 related bugs found in codebase audit                              | ✅ Off-by-one TGID (1-32 vs 0-31 across templates) → name-based lookup; birth_date→age; depth_chart_position vs position; name suffix joining (Mahomes II); duplicate-name contract collision (two Chris Joneses).                           |
| Phase 4: real career stats (Tier 18)        | nflverse stats_player_reg → PCOF / PCDE                                   | ✅ build_stats.py aggregates 1999-2025 parquets into career totals. Brady 66,161 passYds / 488 TDs ✓, Mack 282 tackles / 40 sacks ✓. Per-season tables still empty (Phase 5).                                                                |
...
### Phase 3: real contract import from OvertheCap (optional)

For full historical accuracy, pass `--contracts <path>` to compile-franchise:

```bash
# 1. Build canonical contracts JSONs from nflverse mirror of OvertheCap
python3 scrapers/nflverse/build_contracts.py    # 2008-2026 inclusive

# 2. Build the franchise with real contracts applied
dotnet run --project NcaaDraftEditor.Cli -- compile-franchise \
    out/templates/madden-nfl-08-franchise-template.bin \
    out/franchise-2018.bin \
    --year 2018 \
    --caps data/raw/salary-caps/nfl-salary-caps.json \
    --contracts data/canonical/contracts-2018.json
```

How it works:

1. Each PLAY record in the franchise template is normalized to
   `"FirstName LastName"` (lowercased, punctuation stripped, suffixes
   removed).
2. Looked up against the contracts JSON. If matched, `PCSA` /
   `PSA0..PSA6` / `PSB0..PSB6` / `PSBO` / `PCON` are written from real
   OTC values for that player-year.
3. Unmatched PLAY records (~25-30% in practice, mostly low-OVR backups,
   players with name mismatches between rosters and OTC, undrafted FAs)
   fall through to Phase 2 synthesis.

The compile-franchise CLI prints a `{matched real} / {synthesized fallback}`
breakdown so you can see how the match rate landed.

For the workflow to be useful, the PLAY records need to contain *the
year's players*. The franchise template ships with the disc's launch
year (2007 for M08); for 2018 contracts to match, run `compile-roster`
first to replace PLAY with 2018 player names before compile-franchise:

```bash
# Compile 2018 roster INTO the franchise template
dotnet run --project NcaaDraftEditor.Cli -- compile-roster \
    data/canonical/roster-2018.json \
    data/mappings/positions.json \
    out/templates/madden-nfl-08-franchise-template.bin \
    out/franchise-2018-rostered.bin

# Then run compile-franchise with --contracts on that output
dotnet run --project NcaaDraftEditor.Cli -- compile-franchise \
    out/franchise-2018-rostered.bin \
    out/franchise-2018.bin \
    --year 2018 \
...
engine auto-generate them from the disc-era model.

**Template requirement:** the franchise template MUST be exported from a
truly-fresh franchise (Week 1 preseason, before any games or free-agent
signings). `fetch_m08_franchise_template.py` defaults to this state.
Identifying flags in SEAI for a Week-1 fresh franchise: `SEYR=0 SEWN=0
SEWT=200 SEST=10`. If you use a mid-season or end-of-season template,
the in-game cap UI may display 2007-era simulated values rather than the
era-correct ones we write.
...
ncaa-draft compile-franchise <template.bin> <out.bin>
                            ...
                            [--base-year YYYY] [--no-contracts]       # Phase 1: SEAI.SEYR + SLRI cap.
                            [--contracts <contracts.json>]            # Phase 2: synthesize per-player
                                                                       # contracts (default-on).
                                                                       # Phase 3: --contracts adds real
                                                                       # OTC contract import; matched
                                                                       # players get real values, rest
...
- **M09 / M12 franchise compilers are wired up.** Same `MaddenFranchiseCompiler`
  code path as M08, just with `--base-year 2008` (M09) or `--base-year 2011`
  (M12). Templates come from user-created Week-1-fresh saves on each game's
  memcard; M12 currently sourced from the Deluxe ISO (consistent with the
  M12 roster pipeline).
