...
| 15 | M09 / M12 franchise compiler | ✅ Scaffolding done: `pack_baslus.py` m09-franchise/m12-franchise presets (BASLUS-21770BFran1 / BASLUS-21946BFran1), per-target fetch scripts, `build_all_franchises.py --target m08\|m09\|m12\|all`, build_all.py wires them through. Base years 2008 / 2011. **Blocked on user-provided templates** — must create a fresh Week-1 franchise in M09 and M12 PS2, save to memcard, then run the fetch scripts. Compiler itself is target-agnostic (metadata-driven), so once templates exist the build pipeline should Just Work. |
